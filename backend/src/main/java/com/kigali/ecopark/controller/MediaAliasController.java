...
/**
 * Maps legacy species filenames to unique per-tree media (TREE-001 … TREE-023).
 */
@RestController
public class MediaAliasController {
    ...
    static String resolveCanonicalName(String fileName) {
        ...
        if (lower.startsWith("tree-001-") || lower.startsWith("tree-002-") || lower.startsWith("tree-003-")
                ...
                || lower.startsWith("tree-022-") || lower.startsWith("tree-023-")) {
            return fileName;
        }
        ...
        if (lower.startsWith("entada-abyssinica-022-")) {
            return "TREE-022-" + fileName.substring("entada-abyssinica-022-".length());
        }
        ...
        if (lower.startsWith("bersama-abyssinica-")) {
            return "TREE-023-" + fileName.substring("bersama-abyssinica-".length());
        }
        return fileName;
    }
}
