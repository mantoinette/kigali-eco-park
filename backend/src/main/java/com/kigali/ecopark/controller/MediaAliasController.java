...
/**
 * Maps legacy species filenames to unique per-tree media (TREE-001 / TREE-002).
 * Guarantees Umurehe never plays Umugote audio/video even if the DB still has old URLs.
 */
@RestController
public class MediaAliasController {
    ...
    static String resolveCanonicalName(String fileName) {
        ...
        if (lower.startsWith("tree-001-") || lower.startsWith("tree-002-")) {
            return fileName;
        }
